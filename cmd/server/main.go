...
func SeedDatabase(database *gorm.DB) {
	...
	if count == 0 {
		...
		// 1. Obtener URLs de los microservicios desde el entorno (Fallback a local)
		...
		if pdfURL == "" {
			pdfURL = "http://host.docker.internal:8082" // Puerto del nuevo servicio fp1
		}

		notifyApiURL := notifyURL + "/api/v1"
		pdfApiURL := pdfURL + "/api/v1"

		// 2. Insertar Conectores dinámicamente
		connectors := []domain.Connector{
			{Name: "OTP Generator", Type: "REST", BaseURL: notifyApiURL, AuthType: "NONE"},
			{Name: "WhatsApp Notify", Type: "REST", BaseURL: notifyApiURL, AuthType: "NONE"},
			{Name: "OTP Extractor", Type: "REST", BaseURL: notifyApiURL, AuthType: "NONE"},
			{Name: "OTP Validator", Type: "REST", BaseURL: notifyApiURL, AuthType: "NONE"},
			{Name: "PDF Generator", Type: "REST", BaseURL: pdfApiURL, AuthType: "NONE"}, // Conector 5
		}
		for _, c := range connectors {
			database.Create(&c)
		}

		// 3. Insertar el Flujo 1 con el JSON corregido (El Poka-Yoke validado)
		definition := `{
